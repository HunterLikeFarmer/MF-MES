...
class RFM_RBF():
    """
    rbf(gaussian) kernel of GPy k(x, y) = variance * exp(- 0.5 * x - y_2^2 / lengthscale**2)
    """
...
# def minimize_direct(obj_func, bounds_list, inputs_dim):
...
def minimize(func, start_points, bounds, jac=None, first_ftol=1e-1, second_ftol=1e-2):
    ...
    for i in range(np.shape(x)[0]):
        res = optimize.minimize(func, x0=x[i], bounds=bounds, method='L-BFGS-B', options={'ftol': first_ftol}, jac=jac)
    ...
    if second_ftol < first_ftol:
        ...
        for i in index:
            res = optimize.minimize(func, x0=x[i], bounds=bounds, method='L-BFGS-B', options={'ftol': second_ftol}, jac=jac)
...
def SGD(func, jac, start_points, bounds, tol=1e-4, lr = 1e-1, max_iter=100):
    ...
    for i in range(np.shape(x)[0]):
        ...
        for j in range(max_iter):
            print('j', j)
            derivatives = jac(x[i])
            update = lr / (j+1) * derivatives
            x[i] = x[i] - update
            x[i][x[i] < bounds[0]] = bounds[0][x[i] < bounds[0]]
            x[i][x[i] > bounds[1]] = bounds[1][x[i] > bounds[1]]

            if np.all(update < tol):
                break
...
def initial_design(fir_num, input_dim, bounds):
    if np.size(fir_num) == 1: # Single Fidelity
    ...
    elif np.size(np.unique(fir_num)) == 1:
        temp_initial_points = lhs(input_dim, samples=fir_num[0], criterion='maximin')  * (bounds[1]- bounds[0]) + bounds[0]
        initial_points = [temp_initial_points for _ in range(np.size(fir_num))]
    else:
        print('initial design for M >= 4 and different number for each fidelity is not implemented')
        exit(1)
...
def initial_design_pool(fir_num, input_dim, bounds, pool_X):
    ...
    #     initial_points = [low_initial_points, middle_initial_points, high_initial_points]
        ...
        initial_points = list()
        for i in range(np.size(fir_num)):
            if i == 0:
                tmp_initial_points = lhs(input_dim, samples=fir_num[i], criterion='maximin')  * (bounds[1]- bounds[0]) + bounds[0]
                tmp_initial_points, pool_X[i] = nearest_pool_input(tmp_initial_points, pool_X[i])
            else:
                tmp_initial_points = lhs_subset_greedy(tmp_initial_points, fir_num[i])
                tmp_initial_points, pool_X[i] = nearest_pool_input(tmp_initial_points, pool_X[i])
            initial_points.append(tmp_initial_points)
        ...
        temp_initial_points = lhs(input_dim, samples=fir_num[0], criterion='maximin')  * (bounds[1]- bounds[0]) + bounds[0]
        temp_initial_points, pool_X[0] = nearest_pool_input(temp_initial_points, pool_X[0])

        initial_points = [ temp_initial_points for _ in range(np.size(fir_num))]
        pool_X = [ pool_X[0] for _ in range(np.size(fir_num))]
    else:
        print('initial design for M >= 4 and different number for each fidelity is not implemented')
        exit(1)
...
def _two_inputs_dist(candidates, selected_points):
    ...
    return np.sqrt(norms) # array n \times m
...
def lhs(n, samples=None, criterion=None, iterations=None):
    ...
    if criterion is not None:
        # assert criterion.lower() in ('center', 'c', 'maximin', 'm', 'centermaximin', 'cm', 'correlation', 'corr'), 'Invalid value for "criterion": {}'.format(criterion)
...
def transform_copula(Y):
    Y_size = Y.shape[0]
    # delta_size = 1 / (4 * np.power(Y_size, 0.25) * np.sqrt(np.pi * np.log(Y_size)))
    delta_size = 1. / Y_size
    Y_empirical_dist = np.zeros(Y_size)
    Y_empirical_dist[np.argsort(Y.ravel())] = np.arange(0, Y_size) / Y_size

    # cut off following on [David Salinas et. al., ICML 2020].
...
class GPy_independent_model(object):
    ...
    def predict(self, x, full_cov=False, Y_metadata=None, kern=None, likelihood=None, include_likelihood=True):
        ...
        if np.all( (x[:-1,-1] - x[1:, -1]) > 0 ) and np.shape(x)[0] > 1:
            # assume that x is sorted w.r.t. output_dim (last col)
            print(x[:,-1])
            ...
            for i in range(self.output_dim):
                x_temp = x[x[:,-1]==i, :-1]
                # check the correspond x is not empty
                if np.size(x_temp) > 0:
                    ...
                    if full_cov:
                        if cov is None:
                            ...
                            # non-diagonal matrix is zero from independent assumption
    ...
    # the covariance with same output dim only can be obtained
    def posterior_covariance_between_points(self, X1, X2):
        X1 = np.atleast_2d(X1)
        X2 = np.atleast_2d(X2)
...
class GPy_model_withIntercept(GPy.models.GPRegression):
    ...
    def fit_mean_func(self, X, Y):
        ...
        # coefficients, residual, rank, singular_value = np.linalg.lstsq(A, Y.ravel(), rcond=None)
...
class GPy_model(GPy.models.GPRegression):
    ...
    # return the gradients (regarding x1) of the covariance between two points
    def posterior_covariance_between_points_gradients(self, x1, x2):
        x1 = np.atleast_2d(x1)
        x2 = np.atleast_2d(x2)

        assert np.shape(x1) == np.shape(x2), 'cannot compute diag (not square matrix)'
        Kx2 = self.kern.K(self.X, x2)
        Kx1_grad = self.kern.gradients_X(self.posterior.woodbury_inv.dot(Kx2).T, x1, self.X)
        K12_grad = self.kern.gradients_X(1, x1, x2)

        posterior_cov_gradients = K12_grad - Kx1_grad
        # posterior_cov_gradients = K12_grad - np.einsum('ijk,jl,li->ik', Kx1_grad, self.posterior.woodbury_inv, Kx2)

        # test by numerical differentiation
        # epsilon = 1e-12
        # base = self.posterior_covariance_between_points(x1, x2)
        # print((self.posterior_covariance_between_points(x1 + np.array([[epsilon, 0, 0, 0]]), x2) - base) / epsilon)
        # print((self.posterior_covariance_between_points(x1 + np.array([[0, epsilon, 0, 0]]), x2) - base) / epsilon)
        # print((self.posterior_covariance_between_points(x1 + np.array([[0, 0, epsilon, 0]]), x2) - base) / epsilon)

        return (self.std**2) * posterior_cov_gradients
...
def set_mfgpy_regressor(GPmodel, X_list, Y_list, eval_num, kernel_bounds, M, noise_var=1e-6, optimize_num=10, optimize=True, normalizer=True, opt_separator = None):
    if opt_separator is None:
        opt_separator = OPTIMIZE_SEPARATOR
    fidelity_indexes = np.hstack([i*np.ones(eval_num[i])
                                  for i in range(len(eval_num))])
    X = np.c_[np.vstack([X for X in X_list if np.size(X) > 0]), fidelity_indexes]
    Y = np.vstack([Y for Y in Y_list if np.size(Y) > 0])
    ...
    if GPmodel is None:
        ...
        gp_regressor = GPy_model(X=X, Y=Y, kernel=GPmodel.kern,
                                 noise_var=GPmodel['.*Gaussian_noise.variance'].values, normalizer=GPmodel.normalizer)
        previous_data_num, _ = np.shape(GPmodel.X)
        temp_1 = data_num - data_num % opt_separator # OPTIMIZE_SEPARATOR
        temp_2 = previous_data_num - previous_data_num % opt_separator # OPTIMIZE_SEPARATOR
        if (temp_1 != temp_2 or data_num == previous_data_num)  and optimize:
        # if (temp_1 != temp_2)  and optimize:
            gp_regressor.optimize()
            gp_regressor.optimize_restarts(num_restarts=optimize_num)
...
def mf_model_constrain_cokriging_strictly(GPy_model, kernel_bounds, M, input_dim, noise_var=1e-6):
    ...
    # GPy_model['.*mul.coregion.W'].constrain_bounded(np.sqrt(0.8), 1.)
    # GPy_model['.*mul.coregion.W'].constrain_fixed(1.)
    # GPy_model['.*mul.coregion.kappa'].constrain_fixed(0)
    # GPy_model['.*mul.coregion.kappa'].constrain_bounded(1e-6, 1e-1)
    ...
    # if M==2:
    #     # GPy_model['.*mul_1.coregion.W'].constrain_fixed(0)
    #     GPy_model['.*mul_1.coregion.kappa'][[0]].constrain_bounded(1e-1, 2*1e-1)
    #     GPy_model['.*mul_1.coregion.kappa'][[1]].constrain_bounded(0, 1e-4)
    # elif M==3:
    #     # GPy_model['.*mul_1.coregion.W'][[0]].constrain_bounded(0)
    #     # GPy_model['.*mul_1.coregion.W'][[1]].constrain_bounded(1e-1, np.sqrt(0.1))
    #     # GPy_model['.*mul_1.coregion.W'][[2]].constrain_bounded(1e-1, np.sqrt(0.1))
    #     GPy_model['.*mul_1.coregion.kappa'][[0]].constrain_bounded(1e-1, 2*1e-1)
    #     GPy_model['.*mul_1.coregion.kappa'][[1]].constrain_bounded(1e-1 / 2., 2*1e-1 / 2.)
    #     GPy_model['.*mul_1.coregion.kappa'][[2]].constrain_bounded(0, 1e-4)
    # GPy_model['.*mul_1.coregion.kappa'].constrain_bounded(1e-6, 1e-1)
    return GPy_model
...
def onesided_marginal_density_tmvn(x, mu, sigma, Z, L, upper):
    ...
    #     mu[:marginal_dim], sigma[:marginal_dim, :marginal_dim]).pdf(x)
    pdf = multivariate_normal.pdf(x, mean=mu[:marginal_dim], cov=sigma[:marginal_dim, :marginal_dim])#, allow_singular=True)
...
def main():
    ...
    # upper = np.random.rand(L) * 10
    # lower = - np.random.rand(L) * 10
    # upper = np.zeros(L)
    # lower = -300 * np.ones(L)
    ...
    # start = time.time()
    # mu_2, sigma_2 = ep_tmvn_params(lower, upper, mu, sigma, L)
    ...
    # print(np.linalg.det(sigma_1) / 2)
    # print(np.linalg.det(sigma_2) / 2)
    ...
    for l in range(L):
        ...
        #     cons = Z_ms[i] / Z
        #     alpha = (lower[i, 0] - mu[0]) / sigma[0,0]
        #     beta = (upper[i, 0] - mu[0]) / sigma[0,0]
        #     Z_temp = norm.cdf(beta) - norm.cdf(alpha)
        #     temp = (norm.pdf(alpha) - norm.pdf(beta)) / Z
        #     a = (np.log(np.sqrt(2 * np.pi * np.e) * Z_temp) + (alpha*norm.pdf(alpha) - beta*norm.pdf(beta)) / (2 * Z_temp)) * cons - cons*np.log(cons)
        #     # print(a)
        #     truncated_ent += a
        # print('truncated_ent = ', truncated_ent)
        ...
        if l <= 3:
            ...
            integrate_entropy = integrate_entropy  # / Z + np.log(Z)
            elapsed_time = time.time() - start
            # print ("elapsed_time:{0}".format(elapsed_time) + "[sec]")
