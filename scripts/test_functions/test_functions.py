...
class SynFun_for_ta(test_func):
    ...
    def __init__(self):
        ...
        self.M = 4
        self.bounds = np.r_[np.c_[np.zeros(self.d)].T, np.c_[np.ones(self.d)].T]
        self.cost = np.array([5, 6, 7, 8])
        self.cost = np.array([1, 3, 5, 10])

        self.w_1 = np.sqrt(np.array([[0.7], [0.7], [0.7], [0.7]]))
        self.w_2 = np.sqrt(np.array([[0.3], [0.2], [0.1], [0.1]]))
        self.kappa_1 = np.array([0.1, 0.05, 0.01, 0.01])
        self.kappa_2 = np.array([0.1, 0.05, 0.01, 0.01])
    ...
    def func_sampling(self, ell=None, seed=None):
        ...
        random_X = np.random.rand(1000, self.d)
        self.X = [random_X for _ in range(self.M)]
...
class Bi2O3(test_func):
    def __init__(self):
        ...
        # read the data
        self.data = np.loadtxt('../../../Datas/Bi2O3_and_LLTO/Bi2O3.dat')
        self.X = self.data[:,:self.d]
        self.Y = - self.data[:,self.d:]
        self.bounds = np.c_[np.c_[np.min(self.X, axis=0)], np.c_[np.max(self.X, axis=0)]].T

    def values(self, input, fidelity=None):
        input = np.atleast_2d(input)
        Ys = list()
        for i in range(np.shape(input)[0]):
            temp_index = np.all(self.X == np.atleast_2d(input[i]), axis=1)
            if not np.any(temp_index):
                print('Bi2O3 data does not contain the data with X:', input[i])
                exit()
            Ys.append(self.Y[temp_index,:].ravel())
        Ys = np.array(Ys)
...
class LLTO(test_func):
    def __init__(self):
        self.noise_var = 0
        self.d = 84 + 2101
        self.C = 1
        # self.g_thresholds = 1501.179 * np.ones(self.C) # # of feasible solutions is 6
        self.g_thresholds = 1500.679 * np.ones(self.C) # # of feasible solutions is 40
        # self.g_thresholds = 1500.179 * np.ones(self.C) # # of feasible solutions is 493

        # read the data
        self.data = np.loadtxt('../../../Datas/Bi2O3_and_LLTO/LLTO.dat')
        # used unique data only (2 data have same X respectively, thus 4 data is not used.)
        self.X, index = np.unique(self.data[:,:self.d], return_index=True, axis=0)
        self.Y = - self.data[:,self.d:][index]
        self.bounds = np.c_[np.c_[np.min(self.X, axis=0)], np.c_[np.max(self.X, axis=0)]].T
    ...
    def values(self, input, fidelity=None):
        ...
        for i in range(np.shape(input)[0]):
            temp_index = np.all(self.X == np.atleast_2d(input[i]), axis=1)
            if not np.any(temp_index):
                print('LLTO data does not contain the data with X:', input[i])
                exit()
            Ys.append(self.Y[temp_index,:].ravel())
        Ys = np.array(Ys)
...
class Material(test_func):
    '''
    Material real data Function: d = 2, M = 3
    '''

    def __init__(self):
        self.noise_var = 0
        self.bounds = np.array([[0, -0.25], [0.25, 0]])
        self.d = 2
        self.M = 3
        self.fidelity_features = np.array([5, 10, 60]) # equivarent to Cost
        self.standard_length_scale = standard_length_scale(self.bounds)
        self.maximum = 0
        # read the data
        self.data = np.loadtxt('../../../../Datas/ZQX600_data.csv',delimiter=",",dtype='str',encoding='utf8',usecols=(0,1,2,4,5,6,8,9,10))
        self.data[0][0] = 0.001
        self.data = self.data.astype(float) #String to float

        self.X = [self.data[:,0:2] for i in range(self.M)]
        self.Y = [self.data[:,2], self.data[:,5], self.data[:,8]]
...
class SVM(test_func):
    '''
    SVM real data Function: d = 2, M = 3
    '''

    def __init__(self):
        self.noise_var = 0
        self.bounds = np.array([[-3., -3.], [2., 2.]])
        self.d = 2
        self.standard_length_scale = standard_length_scale(self.bounds)
        self.maximum = 0

        self.first_num_forBO = 3
        # self.M = 4
        # self.first_num = [2, 2, 2, 2]
        # self.data_num_list = [1000, 10000, 20000, 30000]

        self.M = 3
        self.first_num = [3, 2, 1]
        self.data_num_list = [10000, 20000, 30000]

        # read the data
        with open('../../scripts/test_functions/AutoML_data/svm_data/svm_data.pickle', 'rb') as f:
            data = pickle.load(f)


        self.X = [data[:,0:2][data[:,2] == i] for i in self.data_num_list]
        self.Y = [data[:,3][data[:,2] == i] for i in self.data_num_list]
        ...
        self.cost = [ data[:,4][data[:,2] == i] for i in self.data_num_list]

        # self.fidelity_features = np.array([ np.mean(data[:,4][data[:,2] == i]) for i in data_num_list])
        # [34.06765749759674, 138.88091638851165, 478.5935618875504, 1028.3894604384423]
    ...
    def costs(self, input, fidelity=None):
        ...
        return np.c_[self.cost[fidelity][match_index]] / 60.


# 5: learning late {10^-4, 10^-3, 10^-2, 10^-1, 10^0}
# 5: batch size {32, 64, 128, 256, 512}
# 5: (** 2) number of channels: {4, 8, 16, 32, 64}
# 4: drop out rate {1/16, 1/8, 1/4, 1/2}

class cnn_mnist(test_func):
    '''
    CNN MNIST real data Function: d = 5, M = 3
    '''

    def __init__(self):
        self.noise_var = 0
        self.bounds = np.array([[-4., 5., 2., 2., -4.], [0., 9., 6., 6., -1.]])
        self.d = 5
        self.M = 3
        self.standard_length_scale = standard_length_scale(self.bounds)
        self.maximum = 0

        self.first_num_forBO = 4
        self.first_num = [self.first_num_forBO, self.first_num_forBO, self.first_num_forBO]
        # read the data
        with open('../../scripts/test_functions/AutoML_data/cnn_MNIST_data/cnn_MNIST_data.pickle', 'rb') as f:
            data = pickle.load(f)


        self.fidelity_features = np.array([5., 10., 20.])
        self.X = [data[:,0:5][data[:,5] == i] for i in self.fidelity_features]
        self.Y = [data[:,7][data[:,5] == i] for i in self.fidelity_features]

        self.Y = [ np.where(Y <= 0, 1e-5, Y) for Y in self.Y ]
        self.Y = [ np.where(Y >= 1, 1 - 1e-5, Y) for Y in self.Y ]
        self.Y = [ np.log(Y / (1 - Y)) for Y in self.Y ]

        # self.cost = np.array([ np.mean(data[:,6][data[:,5] == i]) for i in self.fidelity_features]) / 60.
        self.cost = [data[:,6][data[:,5] == i] / 60. for i in self.fidelity_features]

        print([np.shape(X) for X in self.X])
        print([np.shape(Y) for Y in self.Y])
        print('mnist cost', self.cost)
    ...
    def values(self, input, fidelity=None):
        ...
        match_index = list()
        for i in range(np.shape(input)[0]):
            tmp_match_index = np.where( np.all(self.X[fidelity] == input[i], axis=1) == True)[0]
    ...
    def costs(self, input, fidelity=None):
        ...
        return np.c_[self.cost[fidelity][match_index]]

class cnn_cifar10(test_func):
    '''
    ...
    def __init__(self):
        ...
        self.first_num = [self.first_num_forBO, self.first_num_forBO, self.first_num_forBO]
        # read the data
        with open('../../scripts/test_functions/AutoML_data/cnn_CIFAR10_data/cnn_CIFAR10_data.pickle', 'rb') as f:
            data = pickle.load(f)
...
class const_cnn_mnist(test_func):
    '''
    CNN MNIST real data Function: d = 5, M = 3
    '''
    ...
    def __init__(self):
        ...
        # read the data
        with open('../../scripts/test_functions/AutoML_data_CBO/cnn_MNIST_data/cnn_MNIST_data.pickle', 'rb') as f:
            data = pickle.load(f)

        self.g_thresholds = 0.9 * np.ones(self.C)
        ...
        print(self.CONST_MAX, self.GLOBAL_MIN)
...
class const_cnn_cifar10(test_func):
    ...
    def __init__(self):
        ...
        # read the data
        with open('../../scripts/test_functions/AutoML_data_CBO/cnn_CIFAR10_data/cnn_CIFAR10_data.pickle', 'rb') as f:
            data = pickle.load(f)
...
def main():
    ...
    # for fun in funcs:
    #     test_func = eval(fun)()
    #     unique_X = np.unique(test_func.X, axis=0)
    #     print('correlation coefficient:\n', np.corrcoef(test_func.Y.T))
    #     print('Data size:', np.shape(unique_X))
    #     print('Deffusion: min={}, max={}, mean={}, std={}'.format(np.min(test_func.Y[:,0]), np.max(test_func.Y[:,0]), np.mean(test_func.Y[:,0]), np.std(test_func.Y[:,0])))
    #     print('Energy: min={}, max={}, mean={}, std={}'.format(np.min(test_func.Y[:,1]), np.max(test_func.Y[:,1]), np.mean(test_func.Y[:,1]), np.std(test_func.Y[:,1])))
    #     feasible_index = test_func.Y[:,-1] > test_func.g_thresholds[0]
    #     print('feasible size', np.shape(test_func.Y[:,-2:][feasible_index]))
    #     optimal_value = np.max(test_func.Y[:,-2][feasible_index])
    #     # print('if func_name==\''+fun+'\':')
    #     # print('    GLOBAL_MAX = ' + str(optimal_value))
    #     # print('    GLOBAL_MIN = ' + str(np.min(test_func.Y[:,-2])))

    #     # distance = np.sum(unique_X**2, axis=1) + np.c_[np.sum(unique_X**2, axis=1)] - 2*unique_X.dot(unique_X.T)
    #     # print(np.median(distance), np.min(distance), np.max(distance))
    #     # for d in range(test_func.d):
    #     #     distance = np.sqrt(np.sum(np.c_[test_func.X[:,d]]**2, axis=1) + np.c_[np.sum(np.c_[test_func.X[:,d]]**2, axis=1)] - 2*np.c_[test_func.X[:,d]].dot(np.c_[test_func.X[:,d]].T))
    ...
    X_list = [X, X]
    test_func = const_SynFun()
    # test_func = const_SynFun_minus_corr()
    # test_func = const_SynFun_test()

    # funcs = ['Gardner1', 'Gardner2', 'Gramacy', 'const_HartMann6']
    # funcs = ['WeldedBeam', 'PressureVessel', 'TensionCompressionString']
    # funcs = ['Test_MESC']
    # funcs = ['G1', 'G4' , 'G6', 'G7', 'G8', 'G9', 'G10', 'G18', 'G24']
    # funcs = ['DTLZ1', 'DTLZ2', 'DTLZ3', 'DTLZ4', 'DTLZ5', 'DTLZ6']
    # funcs = ['DTLZ5', 'DTLZ6']
    funcs = ["HeatExchangerNetworkDesign1", "HeatExchangerNetworkDesign2", "HaverlyPollingProblem",
    "OptimalOperationOfAlkylationUnit", "ReactorNetworkDesign", "WeightedMinimizationOfSpeedReducer", "IndustrialRefrigerationSystem", "Three_barTrussDesign", "MultipleDiskClutchBrakeDesign", "Hydro_staticThrustBearingDesign", "Himmelblau_Function"]
    funcs = ["HaverlyPollingProblem", "Hydro_staticThrustBearingDesign"]
    NUM_SEED = 10
    ell = test_func.ell
    for seed in range(NUM_SEED):
        test_func.func_sampling(ell=ell, seed=seed)
        fun = 'const_SynFun'

    # for fun in funcs:
    #     test_func = eval(fun)()

        bnds = test_func.bounds
        cons_list = list()
        def constraint_maker(c=0):  # c MUST be an optional keyword argument, else it will not work
            def const(x):
                x = np.atleast_2d(x)
                g = test_func.values(x, fidelity=c+1)
                return g[0] - test_func.g_thresholds[c]
            return const


        for c in range(test_func.C):
            cons_list.append({'type': 'ineq', 'fun': constraint_maker(c)})
        cons = tuple(cons_list)



        # plot
        # x = np.linspace(test_func.bounds[0,0], test_func.bounds[1,0], NUM_GRID)
        # X1, X2 = np.meshgrid(x, x)
        # X = np.c_[np.c_[X1.ravel()], np.c_[X2.ravel()]]
        # for c in range(test_func.C + 2):
        #     if c == test_func.C+1:
        #         feasible_index = np.ones((NUM_GRID, NUM_GRID))
        #         for inner_c in range(test_func.C):
        #             const_val = test_func.values(X, fidelity=inner_c+1)
        #             feasible_index[const_val.reshape(NUM_GRID,NUM_GRID) < test_func.g_thresholds[0]] = 0
        #             const_val = const_val.reshape(NUM_GRID,NUM_GRID)
        #             cont=plt.contour(X1, X2, const_val, levels=[test_func.g_thresholds[0]])

        #         # plt.show()
        #         plt.pcolor(X1, X2, feasible_index)
        #         plt.savefig(fun+'_feasible_region.pdf')
        #     elif c > 0:
        #         const_val = test_func.values(X, fidelity=c)
        #         const_val = const_val.reshape(NUM_GRID,NUM_GRID)

        #         plt.pcolor(X1, X2, const_val)
        #         plt.colorbar()
        #         cont=plt.contour(X1, X2, const_val, levels=[test_func.g_thresholds[0]])
        #         cont.clabel(fmt='%1.3f', fontsize=8)
        #         # plt.show()
        #         plt.savefig(fun+'_'+str(c)+'.pdf')
        #     else:
        #         obj_val = test_func.values(X, fidelity=c)
        #         obj_val = obj_val.reshape(NUM_GRID,NUM_GRID)
        #         plt.pcolor(X1, X2, obj_val)
        #         plt.colorbar()
        #         # for i in range(test_func.C):
        #         #     const_val = test_func.values(X, fidelity=i+1)
        #         #     const_val = const_val.reshape(NUM_GRID,NUM_GRID)
        #         #     plt.contour(X1, X2, const_val, levels=[0])
        ...
        if fun == 'const_SynFun':
            func_name = 'const_SynFun_ell='
            # for c in range(test_func.C + 1):
            #     func_name = func_name + '_' + str(test_func.ell[c])
            func_name = func_name + '_' + str(test_func.ell)
            func_name = func_name +'-d='+str(test_func.d)+'-seed'+str(seed)

        # print(test_func.coefficients)
        np.savetxt(func_name+'_coefficients.csv',test_func.coefficients,delimiter=',')
        for c in range(test_func.C+1):
            np.savetxt(func_name+'_weights_'+str(c)+'.csv',test_func.rbf_features_list[c].random_weights,delimiter=',')
            np.savetxt(func_name+'_offset_'+str(c)+'.csv',test_func.rbf_features_list[c].random_offset,delimiter=',')
            # print(test_func.rbf_features_list[c].random_weights)
            # print(test_func.rbf_features_list[c].random_offset)


        # def minus_func(x):
        #     x = np.atleast_2d(x)
        #     f = test_func.values(x, fidelity=0)
        #     return - f[0]
        ...
        # np.random.seed(0)
        # NUM_START = 1000
        # x0 = myutils.lhs(test_func.d, samples=NUM_START, criterion='maximin') * (bnds[1]- bnds[0]) + bnds[0]
        # min_fun = np.inf
        # min_x = None
        # max_fun = -np.inf
        # max_x = None
        # for i in range(np.shape(x0)[0]):
        #     res = scipyminimize(minus_func, x0=x0[i], bounds=bnds.T.tolist(), constraints=cons, method="SLSQP", options={'ftol': 1e-6})
        #     if res['success']:
        #         # check the condition
        #         success_flag = True
        #         for c in range(test_func.C):
        #             if cons[c]['fun'](res['x']) < 0:
        #                 success_flag = False
        #                 break
        #         if max_fun < -res['fun'] and success_flag:
        #             max_fun = -res['fun']
        #             max_x = res['x']

        #     res = scipyminimize(func, x0=x0[i], bounds=bnds.T.tolist(), method="L-BFGS-B", options={'ftol': 1e-4})
        #     if res['success']:
        #         if min_fun > res['fun']:
        #             min_fun = res['fun']
        #             min_x = res['x']
        ...
        #     print('if func_name==\''+ func_name+'\':')
        #     if type(max_fun) is float:
        #         print('    GLOBAL_MAX = ' + str(max_fun))
        #     else:
        #         print('    GLOBAL_MAX = ' + str(max_fun[0]))
        #     print('    GLOBAL_MIN = ' + str(min_fun[0]))

        # elif 'const_SynFun_pool' in fun or 'const_SynFun_test' in fun:
        #     func_name = 'const_SynFun_pool_ell=_'+str(test_func.ell)
    ...
    # for ell in ells:
    #     # plt.figure(figsize=(6, 3*np.int(NUM_SEED)))
    #     for seed in range(NUM_SEED):
    #         # print('ell=', ell, ', seed=', seed)
    #         test_func.func_sampling(ell=ell, seed=seed)
    #         # Y_list = test_func.mf_values(X_list)

    #         def obj_func(x):
    #             x = np.atleast_2d(x)
    #             return -1 * test_func.values(x)
    #         print(myutils.minimize(obj_func, test_func.bounds.T.tolist(), 2))
    #         # exit()

        #     plt_min = np.min(np.vstack(Y_list))
        #     plt_max = np.max(np.vstack(Y_list))

        #     plt.subplot(NUM_SEED, 2, 2*seed+1)
        #     plt.pcolor(X1, X2, Y_list[0].reshape(NUM_GRID, NUM_GRID), cmap='rainbow', vmin=plt_min, vmax=plt_max)
        #     plt.colorbar()
        #     plt.title('m=1: ell='+str(ell)+'_seed='+str(seed))

        #     plt.subplot(NUM_SEED, 2, 2*seed+2)
        #     plt.pcolor(X1, X2, Y_list[1].reshape(NUM_GRID, NUM_GRID), cmap='rainbow', vmin=plt_min, vmax=plt_max)
        #     plt.colorbar()
        #     plt.title('m=2: ell='+str(ell)+'_seed='+str(seed))
        # plt.tight_layout()
        # plt.show()


    # test_funcs = ['Forrester', 'Beale', 'HartMann3', 'HartMann4', 'HartMann6', 'Borehole', 'Branin', 'Colville', 'CurrinExp', 'Styblinski_tang', 'Park1', 'Park2', 'Powell', 'Shekel']

    # for func_name in test_funcs:
    #     print(func_name +'--------------------------------------------------------------------')
    #     test_func = eval(func_name)()
    #     def obj_func(x):
    #         x = np.atleast_2d(x)
    #         return -1 * test_func.values(x)
    #     print(myutils.minimize(obj_func, test_func.bounds.T.tolist(), test_func.d))
...
def find_max_synfun():
    ...
    for seed in range(NUM_SEED):
        ...
        def minus_func(x):
            x = np.atleast_2d(x)
            f = test_func.values(x, fidelity=None)
            return - f[0]

        for i in range(np.shape(x0)[0]):
            res = scipyminimize(minus_func, x0=x0[i], bounds=bnds.T.tolist(), method="L-BFGS-B", options={'ftol': 1e-6})
            if res['success']:
                if min_fun > res['fun']:
                    min_fun = res['fun']
                    min_x = res['x']

        func_name = 'SynFun_ell='+str(ell)+'-d='+str(test_func.d)+'-seed'+str(seed)

        print('if func_name==\''+ func_name+'\':')
        if type(min_fun) is float:
            print('    GLOBAL_MAX = ' + str(-min_fun))
        else:
            print('    GLOBAL_MAX = ' + str(-min_fun[0]))


def find_const_max_synfun_pool():
    func_name = 'const_SynFun_plus_corr_pool'
    func_name = 'const_SynFun_test'
    test_func = eval(func_name)()
    ell = test_func.ell
    ...
    for seed in range(NUM_SEED):
        ...
        def constraint_maker(c=0):  # c MUST be an optional keyword argument, else it will not work
            def const(x):
                x = np.atleast_2d(x)
                g = test_func.values(x, fidelity=c+1)
                return g - test_func.g_thresholds[c]
            return const
        ...
        func_value = func(test_func.X)
        min_f = np.min(func_value)
        # print(np.max(func_value))

        for c in range(test_func.C):
            const_func = constraint_maker(c)
            const_value = const_func(test_func.X)
            func_value[const_value < 0] = - np.inf

        max_f = np.max(func_value)

        # func_value[np.isinf(func_value)] = np.inf
        # print(np.min(func_value))


        func_name_tmp = func_name+'_ell=_'+str(test_func.ell)
        func_name_tmp = func_name_tmp +'-d='+str(test_func.d)+'-seed'+str(seed)

        print('if func_name==\''+ func_name_tmp+'\':')
        print('    GLOBAL_MAX = ' + str(max_f))
        print('    GLOBAL_MIN = ' + str(min_f))
